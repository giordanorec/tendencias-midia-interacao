  ...
  fetch("/futuros/temas.json", { cache: "no-store" }).then(function (r) { return r.json(); }).then(function (lista) {
    lista.forEach(function (a) {
      ...
      var confs = a.confrontos.map(function (c) {
        ...
        return '<div class="tema__confronto">' + (c.imagem ? '<a href="' + a.url + '"><img loading="lazy" src="' + c.imagem + '" alt="Dois mapas do mesmo futuro: ' + esc(c.login) + ' e o professor"></a>' : "") +
          '<p class="kick">Confronto: mapa de ' + esc(c.login) + ' × mapa do professor</p>' + (qs ? '<ol class="tema__perguntas">' + qs + "</ol>" : "") + '</div>';
      }).join("");
      el.innerHTML = '<p class="kick">' + quem + esc(a.estado) + '</p>' + (a.aberto ? confs + '<a class="botao botao--principal" href="' + a.url + '">Página do tema: confronto e ' + a.mapas + ' mapas de futuro</a>' : "");
      el.hidden = false;
    });
  }).catch(function () {});
